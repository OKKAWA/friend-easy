...
public class PluginManagementManager {
    ...
    public static String handlePluginManagement(String json, JavaPlugin hostPlugin) {
    ...
    // region 插件列表功能
    private static List<PluginInfo> getPluginList() {
    ...
    // region 插件操作功能
    private static void validateOperation(PluginOperation op) {
    ...
    private static void executePluginOperation(PluginManager pm, PluginOperation op, JavaPlugin hostPlugin) {
    ...
    private static void checkDependencies(PluginManager pm, Plugin plugin) {
    ...
    // region 通用工具方法
    private static String resolveStatus(int success, int total) {
        if (success == 0) return "failed";
        return success == total ? "complete_success" : "partial_success";
    }

    private static void handleSyntaxError(ProcessingResult result, List<ErrorEntry> errors, Exception e) {
        result.status = "failed";
        result.processed = 0;
        result.total = 0;
    ...
    // region 内部数据结构
    private static class PluginOperationRequest {
        String type;
        String sessionUUID;
        List<PluginOperation> operations;
    }

    private static class PluginOperation {
        String action;
        List<String> plugins;
        boolean force = false;
    }
